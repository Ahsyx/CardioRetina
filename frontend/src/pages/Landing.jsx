...
function AnatomicalHeart({ accent, size = 340 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 300 300" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Main heart body */}
      <path d="M150 240 C150 240 60 185 48 130 C36 75 70 50 95 52 C112 53 128 62 138 75 L150 92 L162 75 C172 62 188 53 205 52 C230 50 264 75 252 130 C240 185 150 240 150 240Z"
        fill="url(#heartFill)" stroke={accent} strokeWidth="1.8" opacity="0.9"/>

      {/* Right atrium */}
      <path d="M205 52 C220 48 240 55 248 70 C255 85 252 105 245 118 C238 131 228 138 218 140 C208 142 198 136 192 125 C185 113 186 98 192 85 C198 72 205 52 205 52Z"
        fill={`${accent}12`} stroke={accent} strokeWidth="1.2" opacity="0.7"/>

      {/* Left atrium */}
      <path d="M95 52 C80 48 60 55 52 70 C45 85 48 105 55 118 C62 131 72 138 82 140 C92 142 102 136 108 125 C115 113 114 98 108 85 C102 72 95 52 95 52Z"
        fill={`${accent}12`} stroke={accent} strokeWidth="1.2" opacity="0.7"/>

      {/* Right ventricle */}
      <path d="M150 145 C165 145 185 148 198 158 C212 168 218 182 215 198 C212 214 200 228 185 236 C170 244 155 245 150 240"
        fill={`${accent}10`} stroke={accent} strokeWidth="1.4" opacity="0.8"/>

      {/* Left ventricle */}
      <path d="M150 145 C135 145 115 148 102 158 C88 168 82 182 85 198 C88 214 100 228 115 236 C130 244 145 245 150 240"
        fill={`${accent}10`} stroke={accent} strokeWidth="1.4" opacity="0.8"/>
      ...
      {/* Aorta */}
      <path d="M162 75 C162 65 165 52 168 42 C171 32 175 24 178 18"
        stroke={accent} strokeWidth="5" strokeLinecap="round" opacity="0.8" filter="url(#glow)"/>
      <path d="M162 75 C162 65 165 52 168 42 C171 32 175 24 178 18"
        stroke={accent} strokeWidth="2.5" strokeLinecap="round" opacity="0.6"/>
      {/* Aortic arch */}
      <path d="M178 18 C185 12 196 10 204 14 C212 18 215 26 212 34"
        stroke={accent} strokeWidth="4.5" strokeLinecap="round" opacity="0.7" filter="url(#glow)"/>
      <path d="M178 18 C185 12 196 10 204 14 C212 18 215 26 212 34"
        stroke={accent} strokeWidth="2" strokeLinecap="round" opacity="0.6"/>

      {/* Pulmonary artery */}
      <path d="M138 75 C136 64 132 50 128 38 C124 26 120 18 116 14"
        stroke={accent} strokeWidth="4" strokeLinecap="round" opacity="0.65" filter="url(#glow)"/>
      <path d="M138 75 C136 64 132 50 128 38 C124 26 120 18 116 14"
        stroke={accent} strokeWidth="1.8" strokeLinecap="round" opacity="0.5"/>
      {/* Pulmonary branches */}
      ...
      {/* Superior vena cava */}
      <path d="M218 80 C228 72 238 65 244 55 C248 48 248 40 244 34"
        stroke={accent} strokeWidth="4" strokeLinecap="round" opacity="0.6"/>

      {/* Inferior vena cava */}
      <path d="M230 160 C244 162 254 168 258 178 C262 188 260 200 254 208"
        stroke={accent} strokeWidth="3.5" strokeLinecap="round" opacity="0.55"/>

      {/* Coronary arteries */}
      <path d="M155 100 C162 108 168 120 165 135 C162 150 155 160 150 165"
        stroke={accent} strokeWidth="1.5" strokeLinecap="round" opacity="0.5" strokeDasharray="2 2"/>
      <path d="M145 100 C138 108 132 120 135 135 C138 150 145 160 150 165"
        stroke={accent} strokeWidth="1.5" strokeLinecap="round" opacity="0.5" strokeDasharray="2 2"/>
      <path d="M150 165 C145 178 140 192 138 208 C136 220 138 232 142 238"
        stroke={accent} strokeWidth="1.2" strokeLinecap="round" opacity="0.4" strokeDasharray="2 3"/>
      <path d="M150 165 C155 178 160 192 162 208 C164 220 162 232 158 238"
        stroke={accent} strokeWidth="1.2" strokeLinecap="round" opacity="0.4" strokeDasharray="2 3"/>
      ...
      {/* Highlight sheen */}
      <path d="M100 90 C108 75 122 68 138 70 C145 71 150 75 150 80"
        stroke="white" strokeWidth="1.5" strokeLinecap="round" opacity="0.12"/>
...
function Landing({ darkMode, setDarkMode }) {
  ...
  const card    = darkMode ? '#0d0d0d' : '#f8f8f8'
  ...
  const steps = [
    ...
    },
    {
      ...
      desc: 'MobileNetV2 scans for vascular changes. Grad-CAM heatmap highlights exact regions of concern on the retina.',
  ...
  return (
    <div style={{ backgroundColor: bg, color: text, minHeight: '100vh', fontFamily: 'Bricolage Grotesque, sans-serif', transition: 'background-color 0.3s', overflowX: 'hidden' }}>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '5rem 4rem 3rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center', minHeight: '85vh' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div style={{ ...a4, display: 'flex', gap: '2rem' }}>
            {[{ v: '88%', l: 'Accuracy' }, { v: '0.91', l: 'AUC-ROC' }, { v: '6', l: 'Biomarkers' }].map((s, i) => (
              <div key={i}>
        ...
        {/* Right — glassmorphism card with anatomical heart */}
          ...
          <div className="heart-float" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <div>
                ...
                <p style={{ margin: 0, fontSize: '1.1rem', fontWeight: '600', fontFamily: 'Junicode, serif' }}>Risk Analysis</p>
              </div>
            ...
            {/* ECG strip inside card */}
            <div style={{ margin: '0.5rem 0', padding: '0.8rem', borderRadius: '12px', backgroundColor: darkMode ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.03)', border: `1px solid ${border}` }}>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '5rem 4rem 2rem' }}>
        <div style={{ marginBottom: '3rem' }}>
          ...
          <h2 style={{ margin: 0, fontSize: '2.2rem', fontFamily: 'Junicode, serif', fontWeight: '400' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1px', backgroundColor: border, borderRadius: '24px', overflow: 'hidden', border: `1px solid ${border}` }}>
          {stats.map((s, i) => (
            <div key={i} className="stat-card" style={{ backgroundColor: card, padding: '2rem', transition: 'transform 0.2s, border-color 0.2s', cursor: 'default', borderBottom: i < 3 ? `1px solid ${border}` : 'none' }}>
              ...
              <div style={{ fontSize: '0.9rem', fontWeight: '600', marginBottom: '0.25rem' }}>{s.label}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
          {steps.map((step, i) => (
            <div key={i} className="step-card" style={{
              ...
              cursor: 'default',
              boxShadow: darkMode ? 'inset 0 1px 0 rgba(255,255,255,0.04)' : 'inset 0 1px 0 rgba(255,255,255,0.9)',
            }}>
              ...
              <h3 style={{ margin: '0 0 0.6rem', fontSize: '1rem', fontWeight: '600' }}>{step.title}</h3>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem 4rem 5rem' }}>
        <div style={{
          ...
          <div>
            ...
            <h2 style={{ margin: '0 0 0.6rem', fontSize: '1.8rem', fontFamily: 'Junicode, serif', fontWeight: '400' }}>
          ...
          <div className="marquee-track">
            {[...techStack, ...techStack].map((tech, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', margin: '0 2.5rem', opacity: 0.4, transition: 'opacity 0.2s', whiteSpace: 'nowrap' }}
                ...
                <img src={`https://cdn.simpleicons.org/${tech.slug}`} alt={tech.name} width="20" height="20" style={{ filter: darkMode ? 'brightness(0) invert(1)' : 'brightness(0)' }}/>
