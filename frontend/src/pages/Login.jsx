...
const PatientIcon = ({ size = 16, color = 'currentColor' }) => (
...
function GooglePermissionDialog({ onConfirm, onCancel, accent, bg, card, text, subtext, border, purpleMode, darkMode }) {
  const permissions = [
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, backgroundColor: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div style={{ backgroundColor: purpleMode ? '#3a0a7a' : darkMode ? '#111111' : '#ffffff', border: `1px solid ${border}`, borderRadius: '28px', padding: '2.5rem 2rem', maxWidth: '400px', width: '100%', boxShadow: purpleMode ? `0 0 0 1px ${accent}22, 0 40px 100px rgba(0,0,0,0.7)` : darkMode ? '0 40px 100px rgba(0,0,0,0.8)' : '0 40px 100px rgba(0,0,0,0.15)', textAlign: 'center', animation: 'dialogIn 0.3s cubic-bezier(0.34,1.56,0.64,1) forwards' }}>
        ...
        <h3 style={{ margin: '0 0 0.5rem', fontSize: '1.1rem', fontWeight: '600', color: text, fontFamily: 'Bricolage Grotesque, sans-serif' }}>Sign in with Google</h3>
        <p style={{ margin: '0 0 1.5rem', fontSize: '0.85rem', color: subtext, lineHeight: 1.7 }}>
          CardioRetina will request access to your <strong style={{ color: text }}>name</strong> and <strong style={{ color: text }}>email address</strong> to create or identify your account.
        </p>
...
function Login({ darkMode, setDarkMode, purpleMode, setPurpleMode }) {
      ...
      if (err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential') {
        try {
          ...
          if (methods.includes('google.com')) {
            toast('This account uses Google sign-in. Please use the "Continue with Google" button.', 'error')
          } else {
            toast('No account found with this email. Please sign up.', 'error')
          }
        } catch {
  ...
  const handleGoogleConfirm = async () => {
    ...
    try {
      ...
      if (snap.exists()) {
        ...
        navigate('/signup', { state: { googleUser: { uid: cred.user.uid, displayName: cred.user.displayName, email: cred.user.email, photoURL: cred.user.photoURL } } })
      }
    } catch (err) {
      if (err.code === 'auth/popup-closed-by-user') toast('Sign-in cancelled.', 'warning')
      else if (err.code === 'auth/popup-blocked') toast('Popup blocked — please allow popups for this site and try again.', 'error')
    ...
    <div style={{ backgroundColor: bg, color: text, minHeight: '100vh', fontFamily: 'Bricolage Grotesque, sans-serif', transition: 'background-color 0.4s, color 0.4s', overflowX: 'hidden' }}>
      ...
      <nav style={{ ...navAnim, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.4rem 4rem', position: 'sticky', top: 0, backgroundColor: `${bg}ee`, backdropFilter: 'blur(12px)', zIndex: 100, borderBottom: `1px solid ${border}`, boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'center' }}>
          ...
          <button onClick={() => navigate('/signup')} className="btn-primary" style={btnPrimary}>Sign Up →</button>
        </div>
      </nav>
      ...
      <section style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '5rem 2rem 3rem', gap: '2rem' }}>
        <div style={{ ...badgeAnim, display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.35rem 1rem', borderRadius: '999px', border: `1px solid ${accent}44`, backgroundColor: `${accent}0d` }}>
          ...
          <span style={{ fontSize: '0.78rem', color: accent, letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: '600' }}>Secure Access</span>
        </div>

        <h2 style={{ ...titleAnim, margin: '-0.5rem 0 0 0', display: 'flex', alignItems: 'baseline', justifyContent: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
          <span style={{ fontSize: 'clamp(2.2rem, 5vw, 3.5rem)', fontWeight: '500', fontFamily: 'Bricolage Grotesque, sans-serif' }}>Welcome</span>
          <span style={{ fontSize: 'clamp(2.5rem, 6vw, 4rem)', fontWeight: '400', fontStyle: 'italic', fontFamily: 'Junicode, serif', color: accent }}>Back</span>
        </h2>

        <p style={{ ...descAnim, fontSize: '1rem', color: subtext, maxWidth: '520px', lineHeight: 1.9, marginTop: '-0.5rem' }}>
          Sign in to view your retinal scans and cardiovascular risk reports.
        </p>

        <div style={{ ...formWrapperAnim, width: '100%', maxWidth: '400px' }}>
          <form onSubmit={handleLogin} style={cardStyle}>
            <div className="stagger-1">
              <label style={{ fontSize: '0.78rem', color: subtext, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.4rem', display: 'block' }}>Email Address</label>
            ...
            <div className="stagger-2">
              <label style={{ fontSize: '0.78rem', color: subtext, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.4rem', display: 'block' }}>Password</label>
              <input type="password" placeholder="Your password" style={inputStyle} required
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', margin: '1.2rem 0' }}>
              ...
              <span style={{ fontSize: '0.72rem', color: subtext, whiteSpace: 'nowrap' }}>or continue with</span>
              <div style={{ flex: 1, height: '1px', backgroundColor: border }}/>
            </div>

            <div className="stagger-4">
              <button type="button" className="google-btn" onClick={() => setShowGoogleDialog(true)} disabled={googleLoading}
                style={{ width: '100%', padding: '0.85rem', borderRadius: '12px', backgroundColor: purpleMode ? 'rgba(255,255,255,0.06)' : darkMode ? '#1a1a1a' : '#f8f8f8', border: `1px solid ${border}`, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', transition: 'all 0.2s', fontFamily: 'Bricolage Grotesque, sans-serif' }}>
            ...
            <div style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.85rem', color: subtext }}>
              Don't have an account?{' '}
              <span onClick={() => navigate('/signup')} style={{ color: accent, cursor: 'pointer', fontWeight: '500' }}>Sign Up</span>
            </div>
          </form>
        </div>
      </section>
      ...
      {showGoogleDialog && (
        <GooglePermissionDialog
          ...
          accent={accent} bg={bg} card={card} text={text} subtext={subtext} border={border}
